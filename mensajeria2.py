...
def manejar_tcp(conn, addr):
    try:
        ...
        if datos.startswith("&file"):
            ruta = datos.split(" ", 1)[1]
            try:
                with open(os.path.basename(ruta), "wb") as f:
                    f.write(conn.recv(4096))
                print(f"{ahora} {addr[0]} <Recibido ./{os.path.basename(ruta)} de ???>")
            except:
                print(f"{ahora} {addr[0]} <Error Recibiendo Archivo de ???>")
        else:
            print(f"{ahora} {datos}")
    finally:
        conn.close()
